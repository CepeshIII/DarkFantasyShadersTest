...
//This example blits the active CameraColor to a new tempRT. It shows how to do a blit with material, and how to use the ResourceData to avoid another blit back to the active color target.
//This example is for API demonstrative purposes. 


// This pass blits the whole screen for a given material to a temp tempRT, and swaps the UniversalResourceData.cameraColor to this temp tempRT.
// Therefor, the next pass that references the cameraColor will reference this new temp tempRT as the cameraColor, saving us a blit. 
...
public class BlitAndSwapColorPass : ScriptableRenderPass
{
    ...
    public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
    {
        ...
        {
            Debug.LogError($"Skipping render pass. BlitAndSwapColorRendererFeature requires an intermediate ColorTexture, we can't use the BackBuffer as a tempRT input.");
            return;
        }
        ...
        //FrameData allows to get and set internal pipeline buffers. Here we update the CameraColorBuffer to the tempRT that we just wrote to in this pass. 
        ...
        //This optimization has some caveats. You have to be careful when the color buffer is persistent across frames and between different cameras, such as in bakingCamera stacking.
        //In those cases you need to make sure your tempRT is an RTHandle and that you properly manage the lifecycle of it.
