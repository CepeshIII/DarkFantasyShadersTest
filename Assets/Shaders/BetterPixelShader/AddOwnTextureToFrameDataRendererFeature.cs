...
public sealed class AddOwnTextureToFrameDataRendererFeature : ScriptableRendererFeature
{
    ...
    public override void Create()
    {
        ...
        if (m_Material == null)
            m_Material = UnityEditor.AssetDatabase.LoadAssetAtPath<Material>("Assets/Shaders/BetterPixelShader/BetterPixell.mat");
#endif
    ...
    private class CustomPostRenderPass : ScriptableRenderPass
    {
        ...
        public CustomPostRenderPass(string passName, Material material, int DefaultSizeDivider/*, int radius*/)
        {
        ...
        private static void ExecuteMainPass(RasterCommandBuffer cmd, RTHandle sourceTexture, Material material)
        {
            ...
            AddOwnTextureToFrameDataVolumeComponent myVolume = VolumeManager.instance.stack?.GetComponent<AddOwnTextureToFrameDataVolumeComponent>();
            if (myVolume != null)
            {
                s_SharedPropertyBlock.SetFloat("_Intensity", myVolume.intensity.value);
                //s_SharedPropertyBlock.SetInt("_Radius", 4);
            }
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
                ...
                AddRasterRenderPass<MainPassData>(passName, out passData, profilingSampler))
            {
                ...
                if (kSampleActiveColor)
                {
                    ...
                    cameraColorDesc.width /= k_DefaultSizeDivider;
                    cameraColorDesc.height /= k_DefaultSizeDivider;

                    passData.outPutTexture = renderGraph.CreateTexture(cameraColorDesc);
                ...
                // Don’t replace cameraColor here — keep the chain intact.
                // Instead, schedule a second blit pass to upscale.
            }
                       ...
                       "UpscaleHalfResToFull", out var copyData, profilingSampler))
            {
                ...
                upscale.SetRenderFunc((CopyPassData data, RasterGraphContext context) =>
                {
